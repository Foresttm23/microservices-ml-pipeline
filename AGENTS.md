...
- Current implementation: all 3 services are minimal FastAPI apps with `GET /` (`gateway/app/main.py`, `orchestrator/app/main.py`, `ml_worker/app/main.py`).
...
- Keep workspace-aware dependency flow: root `pyproject.toml` + `uv.lock`, then `uv sync --frozen` in Docker builder stages.
...
- If startup hangs, inspect missing env/dependencies first (`DB_HOST`, `REDIS_HOST`, Postgres, Redis/RabbitMQ).

## Known Repo-Specific Gotcha
- `gateway/Dockerfile` currently has `RUN chmod +x ml_worker/start.sh`; this likely should target `gateway/start.sh`.
